...
const calculateAverage = (data, field) => {
    if (!data || data.length === 0) return 0;  // If no data, return 0
    ...
    return avg.toFixed(0);  // Return average rounded to 0 decimal places
};
...
const calculateMedian = (data, field) => {
    ...
    if (sortedData.length % 2 === 0) {
        return ((sortedData[middle - 1] + sortedData[middle]) / 2).toFixed(1);  // If even number of items, average the middle two
    }

    return sortedData[middle].toFixed(1);  // If odd, return the middle item
};

// Average and Median component
